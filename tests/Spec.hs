...
singlePullRequestState :: PullRequestId -> Sha -> Branch -> Text -> ProjectState
singlePullRequestState pr prSha prRef prAuthor =
  let event = PullRequestOpened pr prSha prRef prAuthor
  in  handleEventFlat event emptyProjectState

candidateState :: PullRequestId -> Sha -> Branch -> Text -> Sha -> ProjectState
candidateState pr prSha prRef prAuthor candidateSha =
  let state0 = singlePullRequestState pr prSha prRef prAuthor
...
main = hspec $ do
  describe "Logic.handleEvent" $ do
    ...
    it "handles PullRequestOpened" $ do
      let event = PullRequestOpened (PullRequestId 3) (Sha "e0f") (Branch "lol") "lisa"
    ...
    it "handles PullRequestClosed" $ do
      let event1 = PullRequestOpened (PullRequestId 1) (Sha "abc") (Branch "lol") "peter"
          event2 = PullRequestOpened (PullRequestId 2) (Sha "def") (Branch "lol") "jack"
    ...
    it "handles closing the integration candidate PR" $ do
      let event  = PullRequestClosed (PullRequestId 1)
          state  = candidateState (PullRequestId 1) (Sha "ea0") (Branch "lol") "frank" (Sha "cf4")
    ...
    it "does not modify the integration candidate if a different PR was closed" $ do
      let event  = PullRequestClosed (PullRequestId 1)
          state  = candidateState (PullRequestId 2) (Sha "a38") (Branch "lol") "franz" (Sha "ed0")
          state' = handleEventFlat event state
      integrationCandidate state' `shouldBe` (Just $ PullRequestId 2)

    it "loses approval after the PR commit has changed" $ do
      let event  = PullRequestCommitChanged (PullRequestId 1) (Sha "def") (Branch "lol")
          state0 = singlePullRequestState (PullRequestId 1) (Sha "abc") (Branch "lolz") "alice"
    ...
    it "resets the build status after the PR commit has changed" $ do
      let event  = PullRequestCommitChanged (PullRequestId 1) (Sha "def") (Branch "lol")
          state0 = singlePullRequestState (PullRequestId 1) (Sha "abc") (Branch "lolz") "thomas"
    ...
    it "ignores false positive commit changed events" $ do
      let event  = PullRequestCommitChanged (PullRequestId 1) (Sha "000") (Branch "lol")
          state0 = singlePullRequestState (PullRequestId 1) (Sha "000") (Branch "lolz") "cindy"
    ...
    it "sets approval after a comment containing an approval stamp" $ do
      let state  = singlePullRequestState (PullRequestId 1) (Sha "6412ef5") (Branch "lol") "toby"
    ...
    it "does not set approval after a random comment" $ do
      let state  = singlePullRequestState (PullRequestId 1) (Sha "6412ef5") (Branch "lol") "patrick"
    ...
    it "requires a long enough sha for approval" $ do
      let state  = singlePullRequestState (PullRequestId 1) (Sha "6412ef5") (Branch "lol") "sacha"
    ...
    it "handles a build status change of the integration candidate" $ do
      let event  = BuildStatusChanged (Sha "84c") BuildSucceeded
          state  = candidateState (PullRequestId 1) (Sha "a38") (Branch "lol") "johanna" (Sha "84c")
          state' = handleEventFlat event state
          pr     = fromJust $ lookupPullRequest (PullRequestId 1) state'
      buildStatus pr `shouldBe` BuildSucceeded

    it "ignores a build status change of random shas" $ do
      let event0 = PullRequestOpened (PullRequestId 2) (Sha "0ad") (Branch "lol") "harry"
          event1 = BuildStatusChanged (Sha "0ad") BuildSucceeded
          state  = candidateState (PullRequestId 1) (Sha "a38") (Branch "lol") "harry" (Sha "84c")
  ...
  describe "Logic.proceedUntilFixedPoint" $ do
    ...
    it "finds a new candidate" $ do
      let state = setApproval (PullRequestId 1) (Just "fred") $
                  singlePullRequestState (PullRequestId 1) (Sha "f34") (Branch "lol") "sally"
      ...
      actions `shouldBe` [ATryIntegrate (Sha "f34", Branch "lol")]

    it "pushes after a successful build" $ do
      let pullRequest = PullRequest
            {
              sha               = Sha "f35",
              ref               = Branch "voight-kampff",
              author            = "rachael",
    ...
    it "restarts the sequence after a rejected push" $ do
      ...
      buildStatus       pullRequest' `shouldBe` BuildPending
      actions `shouldBe` [APushNewHead (Sha "38d"), ATryIntegrate (Sha "f35", Branch "voight-kampff")]
  ...
  describe "Github.PullRequestPayload" $ do
    ...
    it "should be parsed correctly" $ do
      ...
      let payload    = fromJust maybePayload
          ...
          headRef    = Github.ref        (payload :: PullRequestPayload)
      ...
      headRef    `shouldBe` (Branch "changes")
  ...
  describe "EventLoop.convertGithubEvent" $ do
    ...
    it "should convert a pull request opened event" $ do
      ...
      event `shouldBe` (PullRequestOpened (PullRequestId 1) (Sha "b26354") (Branch "voight-kampff") "rachael")
    ...
    it "should convert a pull request synchronize event" $ do
      let payload = testPullRequestPayload Github.Synchronize
          Just event = convertGithubEvent $ Github.PullRequest payload
      event `shouldBe` (PullRequestCommitChanged (PullRequestId 1) (Sha "b26354") (Branch "voight-kampff"))
