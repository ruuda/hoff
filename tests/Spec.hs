...
main = hspec $ do
  describe "Logic.handleEvent" $ do
    ...
    it "handles PullRequestOpened" $ do
      let event = PullRequestOpened (PullRequestId 3) (Branch "p") (BaseBranch "master") (Sha "e0f") "title" "lisa"
    ...
    it "handles PullRequestClosed" $ do
      let event1 = PullRequestOpened (PullRequestId 1) (Branch "p") (BaseBranch "master") (Sha "abc") "title" "peter"
          event2 = PullRequestOpened (PullRequestId 2) (Branch "q") (BaseBranch "master") (Sha "def") "title" "jack"
    ...
    it "handles PullRequestEdited" $ do
      let event1 = PullRequestOpened (PullRequestId 1) (Branch "p") (BaseBranch "m") (Sha "abc") "title" "peter"
          event2 = PullRequestEdited (PullRequestId 1) "newTitle" (BaseBranch "master")
      ...
      Project.baseBranch pr `shouldBe` (BaseBranch "master")

    it "handles closing the integration candidate PR" $ do
      let event  = PullRequestClosed (PullRequestId 1)
          state  = candidateState (PullRequestId 1) (Branch "p") (BaseBranch "master") (Sha "ea0") "frank" "deckard" (Sha "cf4")
    ...
    it "does not modify the integration candidate if a different PR was closed" $ do
      let event  = PullRequestClosed (PullRequestId 1)
          state  = candidateState (PullRequestId 2) (Branch "p") (BaseBranch "master") (Sha "a38") "franz" "deckard" (Sha "ed0")
          state' = fst $ runAction $ handleEventTest event state
    ...
    it "loses approval after the PR commit has changed" $ do
      let event  = PullRequestCommitChanged (PullRequestId 1) (Sha "def")
          state0 = singlePullRequestState (PullRequestId 1) (Branch "p") (BaseBranch "master") (Sha "abc") "alice"
    ...
    it "resets the approval, integration, and build status after the PR commit has changed" $ do
      let
        state1  = candidateState (PullRequestId 1) (Branch "p") (BaseBranch "master") (Sha "abc") "thomas" "deckard" (Sha "bcd")
    ...
    it "ignores false positive commit changed events" $ do
      let
        ...
        state0 = singlePullRequestState (PullRequestId 1) (Branch "p") (BaseBranch "master") (Sha "000") "cindy"
    ...
    it "sets approval after a stamp from a reviewer" $ do
      let state  = singlePullRequestState (PullRequestId 1) (Branch "p") (BaseBranch "master") (Sha "6412ef5") "toby"
    ...
    it "does not set approval after a comment with the wrong prefix" $ do
      let state  = singlePullRequestState (PullRequestId 1) (Branch "p") (BaseBranch "master") (Sha "6412ef5") "patrick"
    ...
    it "accepts command comments case-insensitively" $ do
      let state  = singlePullRequestState (PullRequestId 1) (Branch "p") (BaseBranch "master") (Sha "6412ef5") "sacha"
          -- Note: "deckard" is marked as reviewer in the test config.
          event  = CommentAdded (PullRequestId 1) "deckard" "@BoT MeRgE"
          state' = fst $ runAction $ handleEventTest event state
          pr     = fromJust $ Project.lookupPullRequest (PullRequestId 1) state'
    ...
    it "handles a build status change of the integration candidate" $ do
      let
        ...
        state  = candidateState (PullRequestId 1) (Branch "p") (BaseBranch "master") (Sha "a38") "johanna" "deckard" (Sha "84c")
        state' = fst $ runAction $ handleEventTest event state
        pr     = fromJust $ Project.lookupPullRequest (PullRequestId 1) state'
    ...
    it "ignores a build status change for commits that are not the integration candidate" $ do
      let
        event0 = PullRequestOpened (PullRequestId 2) (Branch "p") (BaseBranch "master") (Sha "0ad") "title" "harry"
        event1 = BuildStatusChanged (Sha "0ad") Project.BuildSucceeded
        state  = candidateState (PullRequestId 1) (Branch "p") (BaseBranch "master") (Sha "a38") "harry" "deckard" (Sha "84c")
    ...
    it "only checks if a comment author is a reviewer for comment commands" $ do
      let
        state = singlePullRequestState (PullRequestId 1) (Branch "p") (BaseBranch "master") (Sha "a38") "tyrell"
    ...
    it "notifies approvers about queue position" $ do
      let
        state
          = Project.insertPullRequest (PullRequestId 1) (Branch "p") (BaseBranch "master") (Sha "a38") "Add Nexus 7 experiment" (Username "tyrell")
          $ Project.insertPullRequest (PullRequestId 2) (Branch "s") (BaseBranch "master") (Sha "dec") "Some PR" (Username "rachael")
          $ Project.insertPullRequest (PullRequestId 3) (Branch "s") (BaseBranch "master") (Sha "f16") "Another PR" (Username "rachael")
    ...
    it "checks whether pull requests are still open on synchronize" $ do
      let
        state = singlePullRequestState (PullRequestId 1) (Branch "p") (BaseBranch "master") (Sha "b7332ba") "tyrell"
        results = defaultResults
    ...
    it "closes pull requests that are no longer open on synchronize" $ do
      let
        state = singlePullRequestState (PullRequestId 10) (Branch "p") (BaseBranch "master") (Sha "b7332ba") "tyrell"
        results = defaultResults
    ...
    it "does not modify the state on an error during synchronize" $ do
      let
        state = singlePullRequestState (PullRequestId 19) (Branch "p") (BaseBranch "master") (Sha "b7332ba") "tyrell"
    ...
    it "recognizes 'merge and deploy' commands as the proper ApprovedFor value" $ do
      let
        ...
        state = singlePullRequestState prId (Branch "p") (BaseBranch "master") (Sha "abc1234") "tyrell"
    ...
    it "finds a new candidate" $ do
      let
        state
          ...
          $ singlePullRequestState (PullRequestId 1) (Branch "p") (BaseBranch "master") (Sha "f34") "sally"
        results = defaultResults
      ...
      let
        state
          ...
          $ fst $ runAction $ handleEventsTest
            [ PullRequestOpened (PullRequestId 1) (Branch "p") (BaseBranch "master") (Sha "f34") "Untitled" "sally"
            , PullRequestOpened (PullRequestId 2) (Branch "s") (BaseBranch "master") (Sha "g35") "Another untitled" "rachael"
            ] Project.emptyProjectState
        results = defaultResults
          { resultIntegrate = [Right (Sha "38c")]
          , resultPush = [PushRejected]
    ...
    it "pushes after a successful build" $ do
      let
        pullRequest = PullRequest
          ...
          , Project.baseBranch          = BaseBranch "master"
          , Project.sha                 = Sha "f35"
    ...
    it "can handle a rebase failure after a failed push" $ do
      ...
      let
        ...
        results = defaultResults
          { resultIntegrate =
            [ Right $ Sha "b71"
            , Left $ Logic.IntegrationFailure (BaseBranch "master")
            ]
          , resultPush = [ PushRejected ]
          }
  ...
  describe "Github._Payload" $ do
    ...
    it "parses a PullRequestPayload correctly" $ do
      ...
      baseBranch `shouldBe` (BaseBranch "master")
  ...
  describe "EventLoop.convertGithubEvent" $ do
    ...
    it "converts a pull request opened event" $ do
      ...
      event `shouldBe`
        (PullRequestOpened (PullRequestId 1) (Branch "results") (BaseBranch "master") (Sha "b26354") "Add test results" "rachael")
  ...
  describe "ProjectState" $ do
    ...
    it "can be restored exactly after roundtripping through json" $ do
      let
        ...
        singleState = singlePullRequestState (PullRequestId 1) (Branch "p") (BaseBranch "master") (Sha "071") "deckard"
        candState   = candidateState (PullRequestId 2) (Branch "p") (BaseBranch "master") (Sha "073") "rachael" "tyrell" (Sha "079")
    ...
    it "loads correctly after persisting to disk" $ do
      ...
      let fname = tmpBaseDir </> ("state-" ++ (show uuid) ++ ".json")
          state = singlePullRequestState (PullRequestId 1) (Branch "p") (BaseBranch "master") (Sha "071") "deckard"
